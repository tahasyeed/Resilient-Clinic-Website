  ...
  return (
    <div ref={ref} className="relative overflow-hidden bg-gradient-to-br from-[#0c4a6e] to-[#0369a1] rounded-3xl px-6 sm:px-10 md:px-14 lg:px-20 my-16 mx-4 md:mx-10 shadow-2xl">
      ...
      <motion.div className="relative z-10 flex flex-col md:flex-row items-center py-10 md:py-0" initial="hidden" animate={controls} variants={container}>
        <div className="flex-1 py-10 md:py-12 lg:py-14">
          <motion.h1 variants={item} className="text-4xl sm:text-5xl md:text-[3rem] lg:text-[3.5rem] font-bold text-white leading-tight tracking-tight">
            Compassionate Telepsychiatry <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-white">From the Comfort of Home</span>
          </motion.h1>

          <motion.p variants={item} className="text-blue-100/90 mt-4 mb-6 text-lg md:text-xl max-w-lg leading-relaxed">
            Speak with licensed psychiatrists and mental health professionals anytime, anywhere.  
            Get personalized care, therapy, and medication management through secure online sessions.
          </motion.p>
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => { navigate("/appointment"); window.scrollTo(0, 0); }}
              className="bg-white hover:bg-blue-50 text-blue-800 font-semibold px-7 py-3 rounded-full shadow-lg flex items-center gap-2.5"
            >
              Book a Session
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <motion.div className="hidden md:block md:w-1/2 lg:w-[420px] relative min-h-[320px]" variants={item}>
          <img src={assets.about_image} alt="Online mental health consultation" loading="lazy" className="w-full h-auto object-contain translate-y-6 rounded-tl-[100px] rounded-br-2xl border border-blue-900/30 shadow-2xl" />

          <motion.div
            className="absolute top-10 right-0 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-4 w-52 z-20 border border-gray-100/20"
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-blue-900 text-xl">8 mins</p>
                <p className="text-gray-500 text-xs mt-1">Trusted by 10K+ patients</p>
              </div>
            </div>
          </motion.div>

          <motion.div
            className="absolute bottom-14 left-0 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl shadow-2xl p-4 w-60 z-20 text-white"
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.8, duration: 0.8 }}
          >
            <p className="text-sm font-medium">Certified Psychiatrists</p>
            <p className="text-2xl font-bold mt-1">50+</p>
            <p className="text-sm mt-2">Experienced mental health experts</p>
            <div className="w-full bg-white/30 h-px my-2"></div>
            <p className="text-xs flex items-center gap-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
